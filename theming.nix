{
  ...
  # Theme zathura
  # home-manager.users.juliuskoskela.programs.zathura.extraConfig =
  #   builtins.readFile (config.scheme inputs.base16-zathura);

  # Theme `neovim` — more complex, but the principle is the same.
  # home-manager.users.juliuskoskela.programs.neovim = {
  #   plugins = [
  ...
  home-manager.users.juliuskoskela.programs.alacritty.settings.colors = with config.scheme.withHashtag; let
    default = {
      black = base00;
      white = base07;
